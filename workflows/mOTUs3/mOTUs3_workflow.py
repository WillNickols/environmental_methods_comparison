...
def mOTUs(name, paired):
	if paired == "paired":
		command = '''{a}'''.format(
			a = "motus profile -f " + name + "_paired_1." + input_extension + " -r " + name + "_paired_2." + input_extension + " -s " + name + "_unmatched_1." + input_extension + "," + name + "_unmatched_2." + input_extension + " -t " + str(cores) + " -A -o [targets[0]] -n " + name.split("/")[-1]
			)
	if paired == "unpaired":
		command = '''{a}'''.format(
