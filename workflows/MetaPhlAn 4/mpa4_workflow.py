...
def metaphlan4(name, paired):
	if paired == "paired":
		if input_extension == "fastq.gz":
			command = '''{a}'''.format(
				a = "metaphlan " + name + "_paired_1." + input_extension + "," + name + "_paired_2." + input_extension + "," + name + "_unmatched_1." + input_extension + "," + name + "_unmatched_2." + input_extension + " --input_type fastq --output_file " + scratch + name.split("/")[-1] + "_taxonomic_profile.tsv --nproc " + str(cores) + " --no_map --add_viruses --unclassified_estimation --sgb --tmp_dir " + scratch
				)
	if paired == "unpaired":
		if input_extension == "fastq.gz":
			command = '''{a}'''.format(
				a = "metaphlan " + name + "." + input_extension + " --input_type fastq --output_file " + scratch + name.split("/")[-1] + "_taxonomic_profile.tsv --nproc " + str(cores) + " --no_map --add_viruses --unclassified_estimation --sgb --tmp_dir " + scratch
				)
	return str(command)

for name in names:
	if not os.path.isfile(list_targets(name=name, step="metaphlan4", paired=paired)[0]):
		workflow.add_task_gridable(actions=metaphlan4(name, paired=paired),
			depends=list_depends(name=name, step="metaphlan4", paired=paired),
			targets=list_targets(name=name, step="metaphlan4", paired=paired),
			time=calculate_time(name=name, step="metaphlan4", paired=paired),
			mem=memory,
			cores=cores,
			partition=partition
			)
...
merge_depends = []
for name in names:
	merge_depends.append(list_targets(name=name, step="metaphlan4", paired=paired)[0])
