...
def metaphlan3(name, paired):
	if paired == "paired":
		if input_extension == "fastq.gz":
			command = '''{a}'''.format(
				a = "metaphlan " + name + "_paired_1." + input_extension + "," + name + "_paired_2." + input_extension + " --input_type fastq --output_file " + scratch + name.split("/")[-1] + "_taxonomic_profile.tsv --nproc " + str(cores) + " --no_map --add_viruses --unknown_estimation --tmp_dir " + scratch
				)
	if paired == "unpaired":
		if input_extension == "fastq.gz":
			command = '''{a}'''.format(
