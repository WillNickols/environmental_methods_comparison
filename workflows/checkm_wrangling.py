...
data = data.iloc[:, [0, 11, 12, 13]]

data.rename(columns={'Bin Id': "bin_id", "Completeness": "completeness", "Contamination", "contamination", "Strain Heterogeneity", "strain_heterogeneity"})
data = data[~data["bin_id"].str.contains("unbinned|tooShort|lowDepth")]
data['quality'] = np.where(
    data['completeness'] >= 90 and data['contamination'] <= 5, "high_quality", np.where(
    (data['completeness'] >= 90 and data['contamination'] >= 5 and data['contamination'] <= 10) or (data['completeness'] <= 90 and data['completeness'] >= 50 and data['contamination'] <= 10), "medium_quality", "low_quality"))
data['keep'] = np.where(data['completeness'] >= args.completeness and data['contamination'] <= args.contamination, "keep", "reject")

n50 = pd.read_csv(n50, sep='\t', header=0)
n50.rename(columns={'MAG': "bin_id", "N50": "n50"})
n50['bin_id'] = [name.split("/")[-1].split(".")[0] for name in df['bin_id']]
