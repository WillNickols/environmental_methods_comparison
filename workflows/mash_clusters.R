...
'Usage:
   mash_clusters.R [--mash <mash> --checkm <checkm> --mag_dir <mag_dir> --out_dir <out_dir> --threads <threads>]
...
# opts$mash <- "output/bins/mash/mash_dist_out.tsv"
# opts$checkm <- "output/bins/checkm/checkm_qa_n50.tsv"
# opts$out_dir <- "output/bins/"
# opts$threads <- 24
# opts$mag_dir <- "output/bins/qc_bins/"
...
get_reps <- function(clusters) {
  ...
  clusters %>%
    ...
    # rank strain heterogeneity (increasing)
    arrange(strain_heterogeneity) %>%
    mutate(rank_sh = row_number()) %>%
    # rank n50 (decreasing)
    arrange(desc(n50)) %>%
    ...
    # secondary rank
    mutate(rank_secondary = rank_sh + rank_contamination) %>%
...
checkm <- fread(opts$checkm) %>%
  ...
  filter(quality == "high_quality" | quality == "medium_quality")
