...
label_converts <- c('2841037'=3028070, '1404649'=2823807, '2884447'=2792859, '1532'=33035, '1841867'=2897707, 
...
##have to download about 1000 genomesish
all_detected_IDs
write.table(all_detected_IDs, file = "gut_sim_Taxlist.txt", row.names = F, col.names = F, quote=F)


##load in the tree
library(ape)
tree_gut <- read.tree("analysis/databases/Taxonomic_trees/gut_simulation_tree.nwk")

tree_gut$edge.length <- rep(1, nrow(tree_gut$edge))

tree_gut$tip.label <- gsub("[[:punct:] ]", "", tree_gut$tip.label)

missing_IDs <- c()
for(i in profiles){
  missing <- which(is.na(match(i$TaxIDs, tree_gut$tip.label)))
...
distances <- lapply(profiles, function(x) calc_unifrac(truth, x, tree=tree_gut))
...
core_samples <- core_samples %>% filter(Unknown_amount==".up.0.5.mut")
...
core_samples %>% filter(variable %in% c("kraken", "MPA4", "motus", "gtdbtk_megahit", "phylophlan_megahit")) %>% 
  ...
  ggtitle("Taxonomic Distances Gut") +
  theme(axis.text.x=element_text(angle=90)) +
  labs(fill="Method") +
  ylim(0,1)


ggsave("analysis/figures/taxonomic_distances/gut_2B.pdf", height=5, width=5)
