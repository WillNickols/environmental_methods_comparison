...
label_converts <-  c('2841037'=3028070, '29570'=77097, '1404649'=2823807, '2884447'=2792859, '158080'=141390, '50340'=53407, '81037'=77608, 
...
##load in the tree
library(ape)
tree_ocean <- read.tree("analysis/databases/Taxonomic_trees/ocean_simulation_tree.nwk")

tree_ocean$edge.length <- rep(1, nrow(tree_ocean$edge))

tree_ocean$tip.label <- gsub("[[:punct:] ]", "", tree_ocean$tip.label)

missing_IDs <- c()
for(i in profiles){
  missing <- which(is.na(match(i$TaxIDs, tree_ocean$tip.label)))
...
distances <- lapply(profiles, function(x) calc_unifrac(truth, x, tree=tree_ocean))
...
core_samples %>% filter(variable %in% c("kraken", "MPA4", "motus", "gtdbtk_megahit", "phylophlan_megahit")) %>% 
  ...
  ggtitle("Taxonomic Distances Ocean") +
  theme(axis.text.x=element_text(angle=90)) +
  labs(fill="Method") +
  ylim(0,1)
  

ggsave("analysis/figures/taxonomic_distances/ocean_2B.pdf", width=5, height=5)
